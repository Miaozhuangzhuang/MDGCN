...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.res1 = nn.Sequential(
            nn.Conv2d(576, 64, 3, padding=1, dilation=1),
            nn.BatchNorm2d(64),
...
class Regression(nn.Module):
    ...
    def forward(self, x0, x1, x2, x3):
        ...
        y1 = self.res1(y) #B*384*12*12
        y2 = self.res2(y) #B*384*6*6
        y3 = self.res3(y) #B*384*3*3
        return y1,y2,y3
...
class GCN1(nn.Module):
    def __init__(self, in_feats=1):
        ...
        self.Linear1 = nn.Sequential(
            nn.Conv2d(384,1,1),
            nn.ReLU(inplace=True))
        self.gcn = GCN(384, 384, 384)
    def forward(self, y):
        batch = y.shape[0]
        feature = y.shape[1]
        yy = self.Linear(y) # B*1*24*24
        z = yy.reshape(yy.shape[0],-1)
        z = torch.sigmoid(z)
        z = torch.softmax(z, dim=1) #B*144
        src = []
        dst = []
        for j in range(batch):
             ...
             for i in range(144):
                 ...
                 src.append(i+144*j)
                 src.append(i+144*j)
                 dst.append((index[0]+144*j).cpu())
                 dst.append((index[1]+144*j).cpu())                                  
                 dst.append((index[2]+144*j).cpu())
                 dst.append((index[3]+144*j).cpu())
                 dst.append((index[4]+144*j).cpu())
                 dst.append((index[5]+144*j).cpu())
                 dst.append((index[6]+144*j).cpu())
                 dst.append((index[7]+144*j).cpu())
                 dst.append((index[8]+144*j).cpu())
                 dst.append((index[9]+144*j).cpu())
                 dst.append((index[10]+144*j).cpu())
                 dst.append((index[11]+144*j).cpu())
                 if (index[0] != i) and (index[1]!=i) and (index[2]!=i) and (index[3]!=i) and (index[4]!=i) and (index[5]!=i) and (index[6]!=i) and (index[7]!=i) and (index[8]!=i) and (index[9]!=i) and (index[10]!=i) and (index[11]!=i):
                    src.append(i+144*j)
                    dst.append(i+144*j) 
   #     print(src,dst)
        src = np.array(src)
        ...
        adj = dgl.graph((u, v))
     #   print(adj)
        y = y.reshape(y.shape[0],y.shape[1],y.shape[2]*y.shape[3]).transpose(1,2)
        y = y.reshape(y.shape[0]*y.shape[1],y.shape[2])
        y = self.gcn(adj,y)         
        y = y.reshape(batch,144,feature).transpose(1,2)
        y = y.reshape(batch,feature,12,12)
        y = self.Linear1(y)
    ...
    def forward(self, y):
        ...
        for j in range(batch):
             ...
             for i in range(36):
                 ...
                 src.append(i+36*j)
                 src.append(i+36*j)
                 dst.append((index[0]+36*j).cpu())
                 dst.append((index[1]+36*j).cpu())                                  
                 dst.append((index[2]+36*j).cpu())
                 dst.append((index[3]+36*j).cpu())
                 dst.append((index[4]+36*j).cpu())
                 dst.append((index[5]+36*j).cpu())
                 if (index[0] != i) and (index[1]!=i) and (index[2]!=i) and (index[3]!=i) and (index[4]!=i) and (index[5]!=i):
                    src.append(i+36*j)
                    dst.append(i+36*j) 
        src = np.array(src)
        dst = np.array(dst)
     ...
     #   print(adj)
        ...
        y = y.reshape(y.shape[0]*y.shape[1],y.shape[2])
        y = self.gcn(adj,y)         
        y = y.reshape(batch,36,feature).transpose(1,2)
        y = y.reshape(batch,feature,6,6)
        y = self.Linear1(y)
        y = y.reshape(y.shape[0],y.shape[1],-1)
        y = y.sum(dim=2) 
        return y
    ...
    def forward(self, y):
        ...
        for j in range(batch):
             ...
             for i in range(9):
                 ...
                 src.append(i+9*j)
                 src.append(i+9*j)
                 dst.append((index[0]+9*j).cpu())
                 dst.append((index[1]+9*j).cpu())                                  
                 dst.append((index[2]+9*j).cpu())
     ...
     #   print(adj)
        ...
        y = y.reshape(y.shape[0]*y.shape[1],y.shape[2])
        y = self.gcn(adj,y)         
        y = y.reshape(batch,9,feature).transpose(1,2)
        y = y.reshape(batch,feature,3,3)
        y = self.Linear1(y)
        y = y.reshape(y.shape[0],y.shape[1],-1)
        y = y.sum(dim=2) 
        return y
